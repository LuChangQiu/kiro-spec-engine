...
const { buildOntologyFromManifest, validateOntology, queryDependencyChain, getActionInfo, parseDataLineage, getLineageInfo, getAgentHints } = require('../scene-runtime/scene-ontology');
...
async function runScenePackagePublishCommand(rawOptions = {}, dependencies = {}) {
  ...
  try {
    ...
    if (templateDirExists && !options.force) {
...
async function runSceneOntologyValidateCommand(rawOptions = {}, dependencies = {}) {
  ...
  try {
    ...
    const payload = { success: result.valid, packageDir, result };
