...
function buildAutoHandoffRunRecommendations(projectPath, result) {
  ...
  const releaseGatePreflight = result && result.release_gate_preflight && typeof result.release_gate_preflight === 'object'
    ...
    : null;
  if (releaseGatePreflight && releaseGatePreflight.blocked === true) {
    push('kse auto handoff evidence --window 5 --json');
    if (
...
function buildAutoHandoffCapabilityMatrixRecommendations(result = {}) {
  ...
  if (templateDiff.compatibility === 'needs-sync') {
    push(`Sync template library and rerun: kse auto handoff template-diff --manifest ${manifestCli} --json`);
  }
  if (baseline.status === 'error' || (baseline.summary && baseline.summary.portfolio_passed === false)) {
    push('Rebuild Moqui baseline: kse scene moqui-baseline --json');
  }
  ...
  if (coverageSummary && coverageSummary.passed === false) {
    push(
      ...
      `kse auto handoff run --manifest ${manifestCli} --min-capability-coverage ${coverageSummary.min_required_percent} --json`
    );
  }
  if (result.remediation_queue && result.remediation_queue.file) {
    push(
      `Replay remediation queue: kse auto close-loop-batch ${quoteCliArg(result.remediation_queue.file)} --format lines --json`
    );
  }
  ...
  if (coverage.length === 0) {
    ...
    for (const item of coverage) {
      const matchedTemplates = Array.isArray(item && item.matched_templates) && item.matched_templates.length > 0
        ...
        : 'none';
      lines.push(
        `| ${item && item.capability ? item.capability : 'n/a'} | ${item && item.covered === true ? 'yes' : 'no'} | ${matchedTemplates} |`
      );
    }
  }
...
function buildGovernanceRecommendations(summary) {
  ...
  if (!releaseGateAvailable) {
    ...
    if (releaseGatePassed === false || (Number.isFinite(releaseGatePassRate) && releaseGatePassRate < 85)) {
      recommendations.push('Recheck latest release evidence with `kse auto handoff evidence --window 5 --json`.');
    }
    if (
      (Number.isFinite(driftAlertRate) && driftAlertRate > 0) ||
      (Number.isFinite(sceneBatchPassRate) && sceneBatchPassRate < 85)
  ...
  if (!handoffAvailable) {
    recommendations.push(
      ...
      '`kse auto handoff run --manifest docs/handoffs/handoff-manifest.json --dry-run --json`.'
    );
  } else if (handoffTotalRuns > 0) {
    if (
      ...
      recommendations.push('Recheck handoff quality with `kse auto handoff evidence --window 5 --json`.');
      recommendations.push(
        'Resume failed handoff goals with ' +
        '`kse auto handoff run --manifest docs/handoffs/handoff-manifest.json --continue-from latest --continue-strategy failed-only --json`.'
      );
    }
    if (
      (Number.isFinite(handoffFailureRate) && handoffFailureRate > 0) ||
...
function buildAutoGovernanceMaintenancePlan(assessment, policy, dryRun = false) {
  ...
  if (releaseGateBlockState.blocked) {
    releaseGateAdvisoryPlan.push({
      ...
      command: 'kse auto handoff evidence --window 5 --json',
    ...
    if (releaseGateReasons.some(item => `${item}`.includes('scene-batch') || `${item}`.includes('drift'))) {
      releaseGateAdvisoryPlan.push({
        ...
        command: 'kse scene package-publish-batch --manifest docs/handoffs/handoff-manifest.json --dry-run --json',
        blocked_reasons: releaseGateReasons
      });
    }
    if (releaseGateReasons.some(item => `${item}`.startsWith('handoff-'))) {
      releaseGateAdvisoryPlan.push({
        ...
        command: 'kse auto handoff run --manifest docs/handoffs/handoff-manifest.json --dry-run --json',
        blocked_reasons: releaseGateReasons
      });
    }
  }
...
function buildGovernanceCloseLoopRecommendations(finalAssessment, stopReason, stopDetail) {
  ...
  if (reasons.some(item => `${item}`.includes('drift'))) {
    base.push('Review drift blockers with `kse auto handoff evidence --window 5 --json`.');
  }
  if (reasons.some(item => `${item}`.includes('scene-batch'))) {
    base.push(
      'Rerun and stabilize scene batch quality: `kse scene package-publish-batch --manifest docs/handoffs/handoff-manifest.json --dry-run --json`.'
    );
  }
  if (reasons.some(item => `${item}`.startsWith('handoff-'))) {
    base.push('Review handoff quality with `kse auto handoff evidence --window 5 --json`.');
    base.push(
