...
function renderAutoHandoffRegressionMarkdown(payload = {}) {
  ...
  const trendSeriesLines = series.length > 0
    ? series.map(item => {
      ...
      return `- ${sessionId} | ${generatedAt} | risk=${riskLevel} | failed=${failedGoals} | success=${successBar} | ontology=${ontologyBar}`;
    })
    : ['- None'];
...
function buildAutoHandoffRunRecommendations(projectPath, result) {
  ...
  if (
    ...
    push(
      ...
      `kse auto handoff run --manifest ${manifestCli} --require-ontology-validation --json`
    );
...
function buildAutoHandoffReleaseEvidenceEntry(projectPath, result, reportFile = null) {
...
async function mergeAutoHandoffRunIntoReleaseEvidence(projectPath, result, reportFile = null) {
  ...
  const existingSessions = existing && Array.isArray(existing.sessions)
    ...
    : [];
  const nextEntry = buildAutoHandoffReleaseEvidenceEntry(projectPath, result, reportFile);
