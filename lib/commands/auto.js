...
function registerAutoCommands(program) {
  ...
  autoGovernance
    ...
    .action(async (options) => {
      try {
        const result = await runAutoGovernanceCloseLoop(process.cwd(), options);
        if (options.json) {
          console.log(JSON.stringify(result, null, 2));
          return;
        }
...
async function runAutoGovernanceCloseLoop(projectPath, options = {}) {
  const maxRounds = normalizeGovernanceMaxRounds(options.maxRounds, 3);
  const targetRisk = normalizeGovernanceTargetRiskLevel(options.targetRisk);
  const executeAdvisory = Boolean(options.executeAdvisory);
  if (
    !executeAdvisory &&
  ...
  const advisoryRecoverMaxRounds = normalizeGovernanceAdvisoryRecoverMaxRounds(options.advisoryRecoverMaxRounds, 3);
  const advisoryControllerMaxCycles = normalizeGovernanceAdvisoryControllerMaxCycles(options.advisoryControllerMaxCycles, 20);
  ...
  for (let round = 1; round <= maxRounds; round += 1) {
    ...
    if (roundResult.summary.applicable_actions === 0 && !(executeAdvisory && advisoryPlannedActions > 0)) {
