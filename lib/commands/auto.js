...
function buildAutoHandoffRegressionRecommendations(payload = {}) {
  ...
  if (Number.isFinite(moquiMatrixRegressionCount) && moquiMatrixRegressionCount > 0) {
    ...
    push(
      'Run phased remediation from capability clusters first: ' +
      '`node scripts/moqui-matrix-remediation-phased-runner.js --cluster-goals .kiro/auto/matrix-remediation.capability-clusters.json --json`.'
    );
    push('Or use npm alias: `npm run run:matrix-remediation-clusters-phased -- --json`.');
    push(
      'Execute capability-cluster prioritized remediation: ' +
      '`sce auto close-loop-batch .kiro/auto/matrix-remediation.capability-clusters.json --format json --batch-parallel 1 --batch-agent-budget 2 --batch-retry-until-complete --json`.'
    );
    push('Or use npm alias: `npm run run:matrix-remediation-clusters`.');
    push(
      'Then run baseline phased one-shot remediation: ' +
    ...
    push('Or use npm alias: `npm run run:matrix-remediation-from-baseline -- --json`.');
  }
...
function buildAutoHandoffRunRecommendations(projectPath, result) {
  ...
  const pushMoquiClusterFirstRecoverySequence = () => {
    push('Moqui regression recovery sequence (recommended):');
    push(
      `Step 1 (Cluster phased): node scripts/moqui-matrix-remediation-phased-runner.js ` +
      `--cluster-goals ${quoteCliArg(AUTO_HANDOFF_MOQUI_CLUSTER_REMEDIATION_FILE)} --json`
    );
    push('Step 1 alias: npm run run:matrix-remediation-clusters-phased -- --json');
    push(
      `Step 1 fallback (cluster batch): sce auto close-loop-batch ` +
      `${quoteCliArg(AUTO_HANDOFF_MOQUI_CLUSTER_REMEDIATION_FILE)} ` +
      '--format json --batch-parallel 1 --batch-agent-budget 2 --batch-retry-until-complete --json'
    );
    push('Step 1 fallback alias: npm run run:matrix-remediation-clusters');
    push(
      `Step 2 (Baseline phased): node scripts/moqui-matrix-remediation-phased-runner.js ` +
      `--baseline ${quoteCliArg(AUTO_HANDOFF_MOQUI_BASELINE_JSON_FILE)} --json`
    );
    push('Step 2 alias: npm run run:matrix-remediation-from-baseline -- --json');
  };
...
function buildAutoHandoffCapabilityMatrixRecommendations(result = {}) {
  ...
  if (baselineRegressions.length > 0) {
    ...
    push(
      `Execute capability-cluster prioritized remediation: ` +
      `sce auto close-loop-batch ${quoteCliArg(AUTO_HANDOFF_MOQUI_CLUSTER_REMEDIATION_FILE)} ` +
      '--format json --batch-parallel 1 --batch-agent-budget 2 --batch-retry-until-complete --json'
    );
    push('Or use npm alias: npm run run:matrix-remediation-clusters');
    push(
...
async function statsGovernanceCloseLoopSessions(projectPath, options = {}) {
  ...
  for (const session of filteredSessions) {
    ...
    const roundReleaseGateObserved = Number(session && session.round_release_gate_observed);
    if (Number.isFinite(roundReleaseGateObserved) && roundReleaseGateObserved > 0) {
      roundReleaseGateObservedSum += roundReleaseGateObserved;
      const roundReleaseGateChanged = Number(session && session.round_release_gate_changed);
      if (Number.isFinite(roundReleaseGateChanged) && roundReleaseGateChanged > 0) {
  ...
  return {
    ...
    release_gate: {
      ...
      average_pass_rate_percent: releaseGatePassRateCount > 0
        ? Number((releaseGatePassRateSum / releaseGatePassRateCount).toFixed(2))
        : null,
      average_scene_package_batch_pass_rate_percent: releaseGateSceneBatchPassRateCount > 0
        ? Number((releaseGateSceneBatchPassRateSum / releaseGateSceneBatchPassRateCount).toFixed(2))
        : null,
      average_drift_alert_rate_percent: releaseGateDriftAlertRateCount > 0
        ? Number((releaseGateDriftAlertRateSum / releaseGateDriftAlertRateCount).toFixed(2))
        : null,
...
async function loadGovernanceReleaseGateSignals(projectPath) {
  const historyFile = resolveAutoHandoffReleaseGateHistoryFile(projectPath);
  const toNumber = value => {
    if (value === null || value === undefined || value === '') {
      return null;
    }
...
function buildGovernanceRecommendations(summary) {
  ...
  if (!handoffAvailable) {
    ...
    if (
      ...
      recommendations.push(
        'Run phased remediation from capability clusters first with ' +
        '`node scripts/moqui-matrix-remediation-phased-runner.js ' +
        '--cluster-goals .kiro/auto/matrix-remediation.capability-clusters.json --json`.'
      );
      recommendations.push(
        'Or use npm alias: `npm run run:matrix-remediation-clusters-phased -- --json`.'
      );
      recommendations.push(
        'Execute capability-cluster prioritized remediation with ' +
        '`sce auto close-loop-batch .kiro/auto/matrix-remediation.capability-clusters.json ' +
        '--format json --batch-parallel 1 --batch-agent-budget 2 --batch-retry-until-complete --json`.'
      );
      recommendations.push(
      ...
      recommendations.push(
        'Then run baseline phased one-shot remediation with ' +
        '`node scripts/moqui-matrix-remediation-phased-runner.js ' +
        '--baseline .kiro/reports/release-evidence/moqui-template-baseline.json --json`.'
      );
      recommendations.push(
        'Or use npm alias: `npm run run:matrix-remediation-from-baseline -- --json`.'
      );
    }
  }
...
function evaluateGovernanceReleaseGateBlockState(assessment) {
  const snapshot = normalizeGovernanceReleaseGateSnapshot(
    assessment && assessment.health ? assessment.health.release_gate : null
  );
...
function buildGovernanceCloseLoopRecommendations(finalAssessment, stopReason, stopDetail) {
  ...
  if (reasons.some(item => `${item}`.startsWith('handoff-moqui-matrix-regressions-'))) {
    ...
    base.push('Or use npm alias: `npm run run:matrix-remediation-clusters-phased -- --json`.');
    base.push(
      'Execute capability-cluster prioritized remediation with ' +
      '`sce auto close-loop-batch .kiro/auto/matrix-remediation.capability-clusters.json ' +
      '--format json --batch-parallel 1 --batch-agent-budget 2 --batch-retry-until-complete --json`.'
    );
    base.push('Or use npm alias: `npm run run:matrix-remediation-clusters`.');
    base.push(
      'Then run baseline phased one-shot remediation with ' +
      '`node scripts/moqui-matrix-remediation-phased-runner.js ' +
      '--baseline .kiro/reports/release-evidence/moqui-template-baseline.json --json`.'
    );
    base.push('Or use npm alias: `npm run run:matrix-remediation-from-baseline -- --json`.');
  }
