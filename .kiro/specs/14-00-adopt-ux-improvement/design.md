...
|-----------|------|--------|
| No .kiro/ | **Fresh** | Create new structure, no conflicts |
| .kiro/ + same version | **Skip** | Already up-to-date, no action needed |
| .kiro/ + older version | **Smart Update** | Backup + update templates only |
| .kiro/ + newer version | **Warning** | Warn about version mismatch |
| .kiro/ + no version | **Smart Adopt** | Backup + full adoption |
...
📝 Updating files...
  ...
  ⏭️  .kiro/steering/CURRENT_CONTEXT.md (preserved)
...
2. **Smart Update**
   - Existing .kiro/ with old version
   - Updates templates only
   - Preserves user content

3. **Smart Adopt**
   - Existing .kiro/ without version
