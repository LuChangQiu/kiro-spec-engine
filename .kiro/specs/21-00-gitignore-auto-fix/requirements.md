...
**Problem**: 
- Users may have old .gitignore that excludes entire `.kiro/` directory
...
- Backup original .gitignore to `.kiro/backups/gitignore-{timestamp}`
- Include backup ID in result message
- Allow rollback if needed

**FR-2.2.2**: Apply layered .gitignore strategy
- Remove blanket `.kiro/` exclusion if present
...
- ✅ Correctly identifies blanket `.kiro/` exclusion
...
- ✅ Removes blanket `.kiro/` exclusion
- ✅ Adds all required layered rules
- ✅ Preserves non-.kiro rules
...
- Old `.kiro/` exclusion is replaced
...
### Risk 1: User has custom .kiro rules

**Mitigation**: 
- Preserve all user rules
- Only add/update .kiro-related rules
