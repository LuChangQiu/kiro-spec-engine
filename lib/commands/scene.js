...
function registerSceneCommands(program) {
  ...
  sceneCmd
    ...
    .requiredOption('-m, --manifest <path>', 'Path to handoff manifest JSON')
    ...
    .option('--fallback-spec-package <relative-path>', 'Fallback package path inside spec', 'custom/scene-package.json')
    .option('--fallback-scene-manifest <relative-path>', 'Fallback scene manifest path inside spec', 'custom/scene.yaml')
    .option('--out-dir <path>', 'Template library output root', SCENE_PACKAGE_TEMPLATE_DEFAULT_DIR)
...
function normalizeScenePackagePublishBatchOptions(options = {}) {
  return {
    manifest: options.manifest ? String(options.manifest).trim() : undefined,
    status: options.status ? String(options.status).trim().toLowerCase() : 'completed',
    include: normalizePathListOption(options.include).map((item) => String(item).trim()),
    fallbackSpecPackage: normalizeRelativePath(options.fallbackSpecPackage || 'custom/scene-package.json') || 'custom/scene-package.json',
    fallbackSceneManifest: normalizeRelativePath(options.fallbackSceneManifest || 'custom/scene.yaml') || 'custom/scene.yaml',
    outDir: options.outDir ? String(options.outDir).trim() : SCENE_PACKAGE_TEMPLATE_DEFAULT_DIR,
    dryRun: options.dryRun === true,
    force: options.force === true,
    strict: options.strict === true,
...
function validateScenePackageContract(contract = {}) {
  ...
  if (!compatibility) {
    ...
    if (!String(compatibility.kse_version || '').trim()) {
      errors.push('compatibility.kse_version is required');
    }
...
async function runScenePackagePublishBatchCommand(rawOptions = {}, dependencies = {}) {
  ...
  try {
    ...
    const manifestSpecs = Array.isArray(manifest && manifest.specs) ? manifest.specs : [];
    if (manifestSpecs.length === 0) {
      throw new Error('manifest.specs is empty or missing');
    }
    ...
    for (const entry of selected) {
      ...
      if (result && (result.published || result.dry_run === true)) {
        ...
        failures.push({
          spec,
          error: `publish failed (${publishOptions.specPackage}, ${publishOptions.sceneManifest})`
        });
        if (options.strict) {
          break;
        }
