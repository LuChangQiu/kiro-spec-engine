...
class ProjectTimelineStore {
  ...
  async maybeAutoSnapshot(options = {}) {
    ...
    const snapshots = await this._readTimelineEntriesForRead({ limit: 1 });
  ...
  async listSnapshots(options = {}) {
    ...
    const snapshots = await this._readTimelineEntriesForRead({
      trigger,
      limit
    });
  ...
  async _readTimelineEntriesForRead(options = {}) {
    ...
    return this._readTimelineEntriesFromFile(options);
  }
  ...
  async _resolveTimelineEntryBySnapshotId(snapshotId) {
    ...
    return Array.isArray(fromFile) && fromFile.length > 0 ? fromFile[0] : null;
  }
