...
系统通过 `.sce/config/multi-agent.json` 配置文件控制模式：
...
**存储**: `.sce/config/agent-registry.json`
...
**锁文件路径**: `.sce/specs/{specName}/locks/{taskId}.lock`
...
**并发控制策略**:
1. 获取 tasks.md 的文件级锁（`.sce/specs/{specName}/tasks.md.lock`）
2. 读取当前 tasks.md 内容
3. 验证目标行未被修改（行内容比对）
4. 应用变更并原子写入
5. 释放文件级锁
...
**日志文件**: `.sce/config/coordination-log.json`
