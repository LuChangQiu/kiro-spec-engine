...
```bash
# 检查 Requirements 质量门
python template/.kiro/tools/workflow_quality_gate.py requirements <path>

# 检查 Design 质量门
python template/.kiro/tools/workflow_quality_gate.py design <design_path> <requirements_path>

# 检查 Tasks 质量门
python template/.kiro/tools/workflow_quality_gate.py tasks <path>
```
...
```bash
# Requirements 阶段
python template/.kiro/tools/workflow_quality_gate.py requirements .kiro/specs/my-feature/requirements.md
# Exit code: 0 (通过) 或 1 (失败)

# Design 阶段
python template/.kiro/tools/workflow_quality_gate.py design \
    .kiro/specs/my-feature/design.md \
    .kiro/specs/my-feature/requirements.md
# Exit code: 0 (通过) 或 1 (失败)

# Tasks 阶段
python template/.kiro/tools/workflow_quality_gate.py tasks .kiro/specs/my-feature/tasks.md
# Exit code: 0 (通过) 或 1 (失败)
```
...
1. Run: `python template/.kiro/tools/workflow_quality_gate.py requirements <path>`
2. Check exit code:
...
1. Run: `python template/.kiro/tools/workflow_quality_gate.py design <design_path> <requirements_path>`
2. Check exit code:
...
1. Run: `python template/.kiro/tools/workflow_quality_gate.py tasks <path>`
2. Check exit code:
...
```python
from template.kiro.tools.workflow_quality_gate import main as check_quality_gate
import subprocess

# 在创建 requirements.md 后
result = subprocess.run(
    ['python', 'template/.kiro/tools/workflow_quality_gate.py', 
...
**解决**:
1. 从备份恢复：`.kiro/specs/{spec-name}/backups/`
2. 报告 bug，提供文档样本
3. 手动修复文档
...
```bash
# 方法 1: 设置 PYTHONPATH
export PYTHONPATH="${PYTHONPATH}:template/.kiro/tools"
python template/.kiro/tools/workflow_quality_gate.py requirements <path>

# 方法 2: 使用绝对路径
cd template/.kiro/tools
python workflow_quality_gate.py requirements ../../../.kiro/specs/my-feature/requirements.md
```
...
- **质量门控器**: `template/.kiro/tools/quality_gate_enforcer.py`
- **工作流脚本**: `template/.kiro/tools/workflow_quality_gate.py`
- **增强器**: `template/.kiro/tools/ultrawork_enhancer_v3.py`
- **Spec 文档**: `.kiro/specs/04-00-ultrawork-integration-and-quality-automation/`
