...
async function runStudioGenerateCommand(options = {}, dependencies = {}) {
  ...
  const domainChain = job && job.source && job.source.domain_chain
    ...
    : null;
  const domainChainSummary = domainChain && domainChain.summary ? domainChain.summary : null;
  const domainChainContext = domainChain && domainChain.context ? domainChain.context : null;
  const domainChainMetadata = {
    ...
    decision_path_steps: domainChainSummary ? Number(domainChainSummary.decision_path_steps || 0) : 0,
    risk_count: domainChainSummary ? Number(domainChainSummary.risk_count || 0) : 0,
    correction_triggers: domainChainSummary && domainChainSummary.correction_loop
      ? normalizeChainList(domainChainSummary.correction_loop.triggers, 10)
      : [],
    verification_gates: domainChainSummary
      ? normalizeChainList(domainChainSummary.verification_gates, 10)
      : []
  };
