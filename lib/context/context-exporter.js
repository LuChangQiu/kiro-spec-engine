...
${content}`;
  ...
  async includeSteeringRules(projectPath, steeringFiles) {
    const steeringRoots = [
      path.join(projectPath, '.sce/steering'),
    ...
    for (const fileName of steeringFiles) {
      try {
        const filePath = await this._resolveSteeringFilePath(steeringRoots, fileName);
        if (!filePath) {
          continue;
        }
  ...
  /**
   * Resolve steering file from prioritized steering roots.
   *
   * @param {string[]} steeringRoots
   * @param {string} fileName
   * @returns {Promise<string|null>}
   */
  async _resolveSteeringFilePath(steeringRoots, fileName) {
    for (const root of steeringRoots) {
