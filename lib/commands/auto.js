...
function registerAutoCommands(program) {
  ...
  autoHandoff
    ...
    .option('--batch-autonomous', 'Enable autonomous batch policy (default: enabled)', true)
    .option('--continue-on-error', 'Continue remaining goals in close-loop-batch (default: enabled)', true)
...
async function buildAutoHandoffRegressionReport(projectPath, options = {}) {
  ...
  const currentSnapshot = buildAutoHandoffRegressionSnapshot(current);
  if (!previous) {
    return {
      mode: 'auto-handoff-regression',
      current: currentSnapshot,
...
function buildAutoHandoffRunPolicy(options = {}) {
  return {
    ...
    require_ontology_validation: Boolean(options.requireOntologyValidation),
...
function buildAutoHandoffSpecStatus(handoffSpecs = [], batchSummary = null) {
  ...
  const statuses = specs.map(specName => {
    ...
    const batchResult = results.find(item => {
      const goal = `${item && item.goal ? item.goal : ''}`.trim().toLowerCase();
      return goal.startsWith(expectedPrefix);
    }) || null;
    const status = batchResult && typeof batchResult.status === 'string'
      ? batchResult.status
      : 'missing';
    const blockers = batchResult
      ? collectHandoffBlockers(batchResult)
...
function evaluateAutoHandoffRunGates(context = {}) {
  const policy = context.policy || {
    ...
    max_risk_level: 'high',
    require_ontology_validation: false
  };
...
async function runAutoHandoff(projectPath, options = {}) {
  ...
  try {
    ...
    try {
      ...
      if (result.policy.require_ontology_validation && !result.ontology_validation.passed) {
        throw new Error(
          `handoff ontology validation gate failed: ${result.ontology_validation.status || 'missing or not passed'}`
        );
      }
    } catch (error) {
