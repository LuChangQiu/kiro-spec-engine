...
? How to handle conflicts?      # 第二个问题
  > Skip all
    Overwrite all
    Review each
? Overwrite .kiro/steering/CORE_PRINCIPLES.md? (y/N)  # 第三个问题
...
```
...
📦 Analyzing project structure...
✅ Detected existing .kiro/ directory (v1.6.0)

📋 Adoption Plan:
  Mode: Smart Update
  Actions:
    - Backup existing files → .kiro/backups/adopt-20260127-143022/
...
| 检测结果 | 接管模式 | 行为 |
|---------|---------|------|
| 无 .kiro/ 目录 | Fresh | 直接创建，无冲突 |
| 有 .kiro/，版本相同 | Skip | 提示已是最新，无需操作 |
| 有 .kiro/，版本较旧 | Smart Update | 备份 + 更新模板文件 |
| 有 .kiro/，版本较新 | Warning | 警告版本不匹配 |
| 有 .kiro/，无版本信息 | Smart Adopt | 备份 + 完整接管 |
