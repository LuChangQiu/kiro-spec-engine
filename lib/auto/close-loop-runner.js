...
async function runAutoCloseLoop(goal, options = {}, dependencies = {}) {
  ...
  const dodConfig = normalizeDefinitionOfDoneConfig(options);
  const sessionConfig = normalizeSessionConfig(options);
  const replanConfig = normalizeReplanConfig(options);
  ...
  if (sessionConfig.resumeRef) {
    ...
    const requestedSubSpecCount = options.subs !== undefined ? Number(options.subs) : undefined;
  ...
  if (resumedSession) {
    ...
    await writeAgentSyncPlan(projectPath, masterSpecName, runtimeSubSpecs, assignments);
  ...
  if (options.run !== false) {
    const statusReporter = createStatusReporter(options);
    let cycle = 0;
  ...
  await maybeWriteDodReport(result, options, projectPath);
  await maybePersistCloseLoopSession(result, sessionConfig, resumedSession, projectPath);
  await maybePruneCloseLoopSessions(result, sessionConfig, projectPath);
  await maybeWriteOutput(result, options, projectPath);
...
async function resolveCloseLoopSession(projectPath, resumeRef) {
  ...
  const looksLikePath = /[\\/]/.test(resumeRef) || resumeRef.toLowerCase().endsWith('.json');

  if (resumeRef.toLowerCase() === 'latest') {
    filePath = await resolveLatestSessionFile(sessionDir);
  } else if (looksLikePath) {
    const candidate = path.isAbsolute(resumeRef)
      ? resumeRef
      : path.join(projectPath, resumeRef);
    if (await fs.pathExists(candidate)) {
      filePath = candidate;
    }
  } else {
    const byId = path.join(sessionDir, `${sanitizeSessionId(resumeRef)}.json`);
...
function createSessionId(result, sessionConfig) {
  ...
  const prefixToken = Number.isInteger(result.portfolio.prefix)
    ? String(result.portfolio.prefix).padStart(2, '0')
    : 'xx';
  const timestamp = new Date().toISOString().replace(/[-:.TZ]/g, '');
  return `${prefixToken}-${timestamp}`;
}

async function maybePersistCloseLoopSession(result, sessionConfig, resumedSession, projectPath) {
  if (!sessionConfig.enabled) {
    return;
  }

  const sessionDir = getCloseLoopSessionDir(projectPath);
  const sessionId = resumedSession ? resumedSession.id : createSessionId(result, sessionConfig);
  ...
  if (!resumedSession) {
    ...
    snapshot.created_at = resumedSession.data.created_at || snapshot.updated_at;
  }
...
async function materializeReplanCycle(context) {
  ...
  await syncMasterCollaborationMetadata(collabManager, masterSpecName, runtimeSubSpecs);
  await writeAgentSyncPlan(projectPath, masterSpecName, runtimeSubSpecs, runtimeAssignments);
...
async function writeAgentSyncPlan(projectPath, masterSpecName, subSpecs, assignments) {
  ...
  const lines = [
    ...
    '',
    '## Agent Topology',
    ...assignments.map(item => `- \`${item.agent}\`: owns \`${item.spec}\``),
    '',
    '## Dependency Cadence',
    ...subSpecs.map(spec => {
