...
class BootstrapPromptBuilder {
  ...
  async _loadSteeringContext() {
    ...
    for (const filename of STEERING_FILES) {
      const filePath = await this._resolveSteeringFile(filename);
      if (!filePath) {
        continue;
      }
      try {
  ...
  /**
   * Resolve steering file path with `.sce/steering` first, then legacy `.kiro/steering`.
   * @param {string} filename
   * @returns {Promise<string|null>}
   * @private
   */
  async _resolveSteeringFile(filename) {
    for (const relativeDir of STEERING_DIR_CANDIDATES) {
      const candidate = path.join(this._workspaceRoot, relativeDir, filename);
  ...
  async _loadReadmeSummary() {
    ...
    try {
      ...
      return 'SCE (Scene Capability Engine) — Spec-driven development project.';
    }
  }
