...
```mermaid
graph TB
    CLI["CLI 命令层<br/>kse orchestrate run/status/stop"]
...
sequenceDiagram
    ...
    User->>CLI: kse orchestrate run --specs "A,B,C"
...
*对于任何* Spec 名称，BootstrapPromptBuilder 生成的 prompt 应同时包含：该 Spec 的路径（`.sce/specs/{specName}/`）、kse 项目规范/steering 上下文引用、以及任务执行指令。
