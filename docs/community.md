# 社区与交流

欢迎加入 kiro-spec-engine 社区！

---

## 💬 微信交流群

<img src="images/wechat-qr.png" width="300" alt="微信群二维码">

**扫码添加微信，备注"kse"，拉你进群**

---

## 📋 讨论话题

我们的社区讨论以下话题：

### AI 驱动开发
- AI 编程工具使用经验（Claude、Cursor、Copilot 等）
- AI 协作的最佳实践
- Session 上下文管理技巧
- Prompt Engineering 技巧

### Spec 工作流
- Spec 驱动开发方法论
- Requirements、Design、Tasks 编写技巧
- 文档生命周期管理
- 项目组织和结构

### 原则驱动 Agent
- Steering 规则设计
- 原则引擎实践
- Agent 行为优化
- 自动化工作流

### 项目实践
- kiro-spec-engine 使用问题
- 功能建议和反馈
- Bug 报告
- 最佳实践分享

---

## 🌐 其他交流渠道

### GitHub Discussions
https://github.com/heguangyong/scene-capability-engine/discussions

适合：
- 深度技术讨论
- 功能建议
- 最佳实践分享
- 英文交流

### GitHub Issues
https://github.com/heguangyong/scene-capability-engine/issues

适合：
- Bug 报告
- 功能请求
- 技术问题

### 微信公众号
https://mp.weixin.qq.com/s/GRo0XQ6GvQ03T4_FTvAsKA

适合：
- 深度文章阅读
- 关注项目动态
- 中文内容

---

## 📖 延伸阅读

### 核心文章
- [AI 驱动开发的哲学与实践](articles/ai-driven-development-philosophy-and-practice.md)
- [The Philosophy and Practice of AI-Driven Development](articles/ai-driven-development-philosophy-and-practice.en.md)

### 文档
- [快速开始](quick-start.md)
- [Spec 工作流](spec-workflow.md)
- [命令参考](command-reference.md)

### 工具指南
- [Cursor 集成指南](tools/cursor-guide.md)
- [Claude 集成指南](tools/claude-guide.md)
- [Windsurf 集成指南](tools/windsurf-guide.md)

---

## 🤝 贡献

欢迎为 kiro-spec-engine 做出贡献！

- 📝 改进文档
- 🐛 报告 Bug
- 💡 提出新功能
- 🌍 翻译文档
- 💻 贡献代码

查看 [贡献指南](../CONTRIBUTING.md) 了解更多。

---

## 📬 联系方式

- **GitHub**: https://github.com/heguangyong/scene-capability-engine
- **X/Twitter**: https://x.com/heguangyong
- **掘金**: https://juejin.cn/user/heguangyong
- **知乎**: https://www.zhihu.com/people/heguangyong

---

**期待与你交流！** 🚀
