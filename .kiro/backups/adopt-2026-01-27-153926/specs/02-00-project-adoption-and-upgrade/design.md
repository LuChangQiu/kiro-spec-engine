...
**Key Methods**:
- `analyze()`: Scans `.kiro/` directory, checks for specs/, steering/, tools/
...
if (!exists('.kiro/')) {
  return 'fresh';
} else if (!exists('.kiro/version.json')) {
  return 'partial';
} else {
  return 'full';
}
...
**Version File Structure** (`.kiro/version.json`):
```json
{
  "kse-version": "1.0.0",
...
**Backup Metadata** (`.kiro/backups/backup-id/metadata.json`):
```json
{
...
*For any* project directory structure, the detection engine should correctly identify whether `.kiro/` exists, whether `version.json` exists, and which components (specs/, steering/, tools/) are present.
...
*For any* project directory, the adoption mode should be 'fresh' if no `.kiro/` exists, 'partial' if `.kiro/` exists but `version.json` is missing, and 'full' if both exist.
...
*For any* destructive operation (adoption with existing .kiro/, upgrade, or rollback), a valid backup should be created before any changes are made to the project.

**Validates: Requirements 3.2, 5.1**

### Property 7: Backup Integrity

*For any* backup created by the system, validating the backup should confirm that all files from the original `.kiro/` directory are present and readable in the backup.
...
*For any* valid backup, rolling back to that backup should restore the `.kiro/` directory to the exact state it was in when the backup was created.
...
Details:
  - Existing file: .kiro/steering/CORE_PRINCIPLES.md
...
describe('Fresh Adoption', () => {
  test('creates complete .kiro/ structure', async () => {
    const projectPath = await createEmptyProject();
    
    const result = await adoptCommand({ auto: true });
    
    expect(result.success).toBe(true);
    expect(fs.existsSync(path.join(projectPath, '.kiro'))).toBe(true);
    expect(fs.existsSync(path.join(projectPath, '.kiro/version.json'))).toBe(true);
    expect(fs.existsSync(path.join(projectPath, '.kiro/steering'))).toBe(true);
    expect(fs.existsSync(path.join(projectPath, '.kiro/tools'))).toBe(true);
  });
  
  test('handles existing .kiro/ directory gracefully', async () => {
...
- [ ] Test with large projects (1000+ files in .kiro/)
