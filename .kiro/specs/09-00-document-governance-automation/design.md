...
class ValidationEngine {
  ...
  async validateSpec(specName) {
    const specPath = path.join(this.projectPath, '.kiro/specs', specName);
...
*For any* report generation operation, a markdown file should be created at `.kiro/reports/document-compliance-{timestamp}.md` containing the compliance report.
...
**Unit Test Scenarios:**
1. Empty project (no .kiro directory)
...
Store configuration in `.kiro/config/docs.json`:
...
Store execution history in `.kiro/logs/governance-history.json`:
...
**Version Detection:**
- Check for existing .kiro directory structure
...
1. `.kiro/config/docs.json` - Document governance settings
