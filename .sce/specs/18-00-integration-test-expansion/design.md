...
class IntegrationTestFixture {
  ...
  async createDefaultConfig() {
    // Create minimal .sce/config.json
  }
...
**Test Cases**:
1. Adopt new project and verify .sce structure
...
describe('Project Lifecycle Integration', () => {
  test('should adopt project and create .sce structure', async () => {
...
describe('Operations Integration', () => {
  test('should generate documentation for spec', async () => {
    ...
    expect(result.exitCode).toBe(0);
    expect(await fixture.fileExists('.sce/docs/test-spec.md')).toBe(true);
  });
});
```
...
```javascript
{
  ...
  kiroDir: string,            // .sce directory path
...
node .sce/specs/17-00-test-suite-optimization/scripts/coverage-analyzer.js
```
