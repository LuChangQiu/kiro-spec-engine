...
Automatically detect and fix .gitignore files during kse adoption and upgrade to ensure team collaboration works correctly. The system replaces old blanket `.kiro/` exclusion patterns with a layered strategy that commits Specs while excluding personal state.
...
- Identify old patterns (blanket `.kiro/` exclusion)
...
- Store in `.kiro/backups/gitignore-{timestamp}`
...
```javascript
{
  ...
  oldPatterns: string[],     // Old patterns found (e.g., ['.kiro/', '.kiro/*'])
...
2. **Old Pattern**: Contains `.kiro/` or `.kiro/*` → Strategy: `update`
...
2. **Update Strategy** (old pattern or incomplete):
   - Remove old blanket patterns (`.kiro/`, `.kiro/*`)
   - Remove incomplete layered rules (if any)
   - Add complete layered section
   - Preserve all non-.kiro rules

**Preservation Rules**:

- Keep all rules not related to `.kiro/`
- Maintain rule order where possible
- Preserve comments (except in kse-managed section)
...
- Keep metadata in `.kiro/backups/gitignore-{timestamp}.meta.json`
...
```javascript
{
  pattern: string,           // Rule pattern (e.g., '.kiro/backups/')
  type: string,              // 'exclusion' | 'negation' | 'comment'
  line: number,              // Line number in file
  isKiroRelated: boolean,    // Rule relates to .kiro/
  isManaged: boolean         // Rule is in kse-managed section
}
```
...
*For any* .gitignore content containing patterns like `.kiro/`, `.kiro/*`, or `.kiro/**`, the detector should identify it as having an old pattern.
...
*For any* .gitignore content containing blanket `.kiro/` exclusion patterns, after transformation the result should not contain those patterns.
...
*For any* .gitignore content with non-.kiro rules, after transformation all those user rules should still be present in the result.
...
- Provide manual fix instructions
- Allow retry with `kse doctor --fix-gitignore`
...
2. Run: kse doctor --fix-gitignore
3. Manual fix: See docs/team-collaboration-guide.md

Backup location: .kiro/backups/gitignore-[timestamp]
```
...
1. **Specific Examples**:
   ...
   - Old pattern `.kiro/` → Replace with layered rules
   - Compliant .gitignore → No changes

2. **Edge Cases**:
   - Missing .gitignore file
   - .gitignore with only comments
   - .gitignore with negation rules (`!.kiro/specs/`)
...
const oldPatternGitignore = fc.record({
  oldPattern: fc.constantFrom('.kiro/', '.kiro/*', '.kiro/**'),
...
**Backup Security**:
- Store backups in `.kiro/backups/` (gitignored)
...
- Detect old kse-managed sections
...
2. **Custom Rules**:
   - Allow users to add custom .kiro rules
   - Preserve custom rules during updates
   - Support `.kse.config.json` for custom patterns
...
5. **Rollback UI**:
   - `kse doctor --list-gitignore-backups`
   - `kse doctor --restore-gitignore <backup-id>`
