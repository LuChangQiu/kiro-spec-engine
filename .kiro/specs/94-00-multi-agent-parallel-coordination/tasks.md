...
- [x] 1. 多 Agent 配置与模式检测
  - [x] 1.1 创建 `lib/collab/multi-agent-config.js`
    - 实现 `MultiAgentConfig` 类，负责读取/写入 `.kiro/config/multi-agent.json`
...
- [x] 2. Agent 注册表
  - [x] 2.1 创建 `lib/collab/agent-registry.js`
    - 实现 `AgentRegistry` 类，管理 `.kiro/config/agent-registry.json`
...
- [x] 4. 任务级锁管理器
  - [x] 4.1 创建 `lib/lock/task-lock-manager.js`
    ...
    - 实现 `acquireTaskLock(specName, taskId, agentId)`：创建 `.kiro/specs/{specName}/locks/{taskId}.lock`
...
- [x] 5. 任务状态安全存储
  - [x] 5.1 创建 `lib/task/task-status-store.js`
    ...
    - 文件锁路径：`.kiro/specs/{specName}/tasks.md.lock`
    - 写入前验证目标行未被修改（行内容比对）
