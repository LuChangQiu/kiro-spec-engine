...
The steering directory compliance check is a pre-execution validation system that ensures the `.kiro/steering/` directory contains only approved files and no subdirectories. This prevents context pollution and excessive token consumption in AI sessions.
...
The .kiro/steering/ directory contains files or subdirectories that are not allowed.
...
Fix Suggestions:
  • Move analysis reports to: .kiro/specs/{spec-name}/results/
  • Move historical data to: .kiro/specs/{spec-name}/
...
**Scenario**: `.kiro/steering/` directory doesn't exist
...
```javascript
const steeringPath = path.resolve(projectRoot, '.kiro/steering');
