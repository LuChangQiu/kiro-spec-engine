...
- **Program governance stabilization loop**: Added `close-loop-program` governance controls (`--program-govern-until-stable`, `--program-govern-max-rounds`, `--program-govern-max-minutes`, anomaly knobs) so gate/anomaly failures can trigger bounded replay/recover rounds until stable, with `program_governance`, `program_kpi_trend`, and `program_kpi_anomalies` telemetry.
