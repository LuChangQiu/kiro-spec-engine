...
- **Environment_Registry**: The persistent storage of all registered environments (`.kiro/environments.json`)
...
3. WHEN creating backups, THE Backup_System SHALL store them in a `.kiro/env-backups/` directory with timestamp and environment name
...
4. THE Environment_Manager SHALL store workspace-specific environment registries in `.kiro/workspaces/{workspace-name}/environments.json`
