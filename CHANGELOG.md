...
- **Labeled dual-command recovery block for handoff run**: `sce auto handoff run` recommendations now emit explicit `Step 1 (Cluster phased)` and `Step 2 (Baseline phased)` commands to remove execution ambiguity.
