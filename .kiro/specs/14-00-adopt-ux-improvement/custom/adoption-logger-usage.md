...
// Creates log file at: /path/to/project/.kiro/logs/adopt-20260127-143022.log
```
...
logger.debug('Checking file permissions', {
  file: '.kiro/steering/CORE_PRINCIPLES.md',
  readable: true,
  writable: true
});
...
logger.startOperation('backup', {
  ...
  destination: '.kiro/backups/adopt-20260127-143022'
});
...
// Log file operations
logger.fileOperation('create', '.kiro/steering/ENVIRONMENT.md', {
  size: 2048,
  source: 'template'
});

logger.fileOperation('update', '.kiro/steering/CORE_PRINCIPLES.md', {
  oldVersion: '5.0',
  newVersion: '5.1'
});

logger.fileOperation('preserve', '.kiro/specs/my-feature/requirements.md', {
  reason: 'User content'
});
```
...
Log files are created at `.kiro/logs/adopt-{timestamp}.log` with the following format:
...
[2026-01-27T14:30:22.123Z] [+0ms] [INFO] Logger initialized {"logFile":".kiro/logs/adopt-20260127-143022.log"}
...
```
{projectPath}/.kiro/logs/adopt-{timestamp}.log
```

Example:
```
/path/to/project/.kiro/logs/adopt-20260127-143022.log
```
