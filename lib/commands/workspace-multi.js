...
const WorkspaceRegistry = require('../workspace/multi/workspace-registry');
const GlobalConfig = require('../workspace/multi/global-config');
const WorkspaceContextResolver = require('../workspace/multi/workspace-context-resolver');
...
async function createWorkspace(name, options = {}) {
  ...
  try {
    ...
    // Create workspace
    const workspace = await registry.createWorkspace(name, workspacePath);
...
async function switchWorkspace(name, options = {}) {
  const registry = new WorkspaceRegistry();
  const config = new GlobalConfig();
  const resolver = new WorkspaceContextResolver(registry, config);
  
  try {
    console.log(chalk.red('🔥') + ' Switching Workspace');
    console.log();
    
    // Set active workspace (this validates existence and updates timestamp)
    await resolver.setActiveWorkspace(name);
    
    const workspace = await registry.getWorkspace(name);
...
async function removeWorkspace(name, options = {}) {
  ...
  try {
    ...
    const workspace = await registry.getWorkspace(name);
    if (!workspace) {
      const available = await registry.listWorkspaces();
    ...
    // Remove workspace
    await registry.removeWorkspace(name);
...
async function infoWorkspace(name = null, options = {}) {
  ...
  try {
    ...
    if (!name) {
      workspace = await resolver.getActiveWorkspace();
      ...
      workspace = await registry.getWorkspace(name);
