...
*For any* template application, all template files (requirements.md, design.md, tasks.md) must be copied to the target Spec directory with the correct directory structure (.kiro/specs/<name>/).
...
describe('Integration Tests', () => {
  test('complete template application workflow', async () => {
    ...
    expect(fs.existsSync('.kiro/specs/my-feature/requirements.md')).toBe(true);
    expect(fs.existsSync('.kiro/specs/my-feature/design.md')).toBe(true);
    expect(fs.existsSync('.kiro/specs/my-feature/tasks.md')).toBe(true);
    
    // Verify: No frontmatter in created files
    const content = fs.readFileSync('.kiro/specs/my-feature/requirements.md', 'utf8');
  ...
  test('offline mode with cached templates', async () => {
    ...
    expect(fs.existsSync('.kiro/specs/offline-feature/requirements.md')).toBe(true);
  });
});
```
