...
- **Steering_File**: `.kiro/steering/` 目录下的配置文件，如 CURRENT_CONTEXT.md
...
1. WHEN 一个 Agent 请求锁定某个任务，THE Task_Lock SHALL 创建该任务的独立锁文件（`.kiro/specs/{spec}/locks/{taskId}.lock`）
...
4. WHEN 重试次数耗尽仍无法获取锁，THE Task_Status_Store SHALL 将更新内容写入临时文件（`.kiro/steering/{filename}.pending.{agentId}`）供后续合并
