...
4. **Current directory contains `.kiro/` (single-project mode)**
...
class WorkspaceContextResolver:
    ...
    def is_valid_kse_directory(self, path: str) -> bool:
        """Check if directory contains .kiro/ structure"""
...
*For any* valid workspace name and path containing a `.kiro/` directory, creating a workspace should result in the registry containing an entry with that name, path, and valid timestamps.
...
*For any* path that does not contain a `.kiro/` directory, attempting to create a workspace with that path should return an error and not modify the registry.
...
Details: Path '/home/user/invalid' does not contain a .kiro/ directory
