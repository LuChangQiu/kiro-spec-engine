...
describe('WorkspaceStateManager', () => {
  ...
  describe('Legacy Migration', () => {
    it('should migrate from legacy format', async () => {
      ...
      const legacyConfig = {
        version: '1.0',
        active_workspace: 'legacy-ws',
        preferences: {
          auto_detect_workspace: false,
      ...
      await fs.writeFile(
        ...
        JSON.stringify(legacyWorkspaces),
        'utf8'
      );
      ...
      // Create manager with path in same directory
      const newStatePath = path.join(legacyDir, 'workspace-state.json');
      const migrationManager = new WorkspaceStateManager(newStatePath);

      // Override hasLegacyFiles to check temp directory
      migrationManager.hasLegacyFiles = async function() {
        const legacyWorkspacesPath = path.join(legacyDir, 'workspaces.json');
      ...
      // Override migrateFromLegacy to use temp directory
      migrationManager.migrateFromLegacy = async function() {
        const legacyWorkspacesPath = path.join(legacyDir, 'workspaces.json');
        const legacyConfigPath = path.join(legacyDir, 'config.json');
        ...
        const Workspace = require('../../../lib/workspace/multi/workspace');
        ...
        if (await fs.pathExists(legacyWorkspacesPath)) {
          const backupPath = path.join(legacyDir, `workspaces.json.backup-${timestamp}`);
          await fs.copy(legacyWorkspacesPath, backupPath);
          await fs.remove(legacyWorkspacesPath);
        }
        if (await fs.pathExists(legacyConfigPath)) {
          const backupPath = path.join(legacyDir, `config.json.backup-${timestamp}`);
      ...
      // Verify migration
      expect(migrationManager.state.activeWorkspace).toBe('legacy-ws');
      expect(migrationManager.state.workspaces.size).toBe(1);
      expect(migrationManager.state.preferences.autoDetectWorkspace).toBe(false);

      // Verify new file exists
      expect(await fs.pathExists(newStatePath)).toBe(true);

      // Verify legacy files are backed up
