  ...
  if (recentEntries.length === 0) {
    ...
    recentEntries.forEach(entry => {
      ...
      lines.push(
        ...
        `scene-failures=${sceneBatchFailures} | drift-alerts=${driftAlerts} | drift-blocked=${driftBlocked} | ` +
...
async function buildAutoGovernanceStats(projectPath, options = {}) {
  ...
  const [sessionStats, batchStats, controllerStats, recoveryMemory, releaseGateSignals] = await Promise.all([
