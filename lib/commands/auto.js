...
function registerAutoCommands(program) {
  ...
  applyCloseLoopOptions(
    ...
    .action(async (goal, options) => {
      try {
        if (!options.resume && (!goal || `${goal}`.trim() === '')) {
          throw new Error('Goal is required unless --resume is provided.');
        }
        const result = await runAutoCloseLoop(goal, options);
  ...
  applyCloseLoopOptions(
    ...
    .option('--batch-parallel <n>', 'Maximum goals to run concurrently in batch mode (default: 1)', parseInt)
    .option('--batch-agent-budget <n>', 'Global parallel-agent budget shared across all batch goals', parseInt)
    .option('--batch-priority <strategy>', 'Batch goal priority strategy: fifo|complex-first|complex-last|critical-first (default: fifo)', 'fifo')
    .option('--batch-aging-factor <n>', 'Aging boost per scheduling cycle for waiting goals (default: 0)', parseInt)
    .option('--batch-retry-rounds <n>', 'Automatically retry failed/stopped goals for N extra rounds (default: 0)', parseInt)
    ...
    .option('--batch-autonomous', 'Enable autonomous batch policy (continue-on-error + adaptive parallel + retry-until-complete)')
  ...
  applyCloseLoopOptions(
    ...
    .option('--batch-autonomous', 'Enable autonomous batch policy explicitly (already default for close-loop-program)')
  ...
  auto
    ...
    .option('--dequeue-limit <n>', 'Maximum goals to consume from queue per controller cycle (default: 1)', parseInt)
  ...
  applyCloseLoopOptions(
    ...
    .option('--batch-retry-until-complete', 'Keep retrying failed/stopped goals until completed or retry max is reached')
    .option('--batch-retry-max-rounds <n>', 'Max extra rounds for retry-until-complete mode (default: 10)', parseInt)
    .option('--batch-autonomous', 'Enable autonomous batch policy explicitly (default: enabled in recovery)')
    .option('--batch-session-id <id>', 'Set explicit batch session id for summary persistence')
  ...
  autoKpi
    .command('trend')
    .description('Aggregate periodic KPI trend from persisted batch/program/recover summaries')
    .option('--weeks <n>', 'Number of recent weeks to include (default: 8)', parseInt)
    .option('--mode <mode>', 'Summary mode filter: all|batch|program|recover (default: all)', 'all')
...
function applyCloseLoopOptions(command, options = {}) {
  command
    ...
    .option('--resume <session-or-file>', 'Resume close-loop from a prior session id, "latest", or a JSON file path')
...
function resolveBatchAutonomousPolicy(options, goalsCount = 0) {
  const enabled = Boolean(options && options.batchAutonomous);
  const baseOptions = { ...options };
  if (!enabled) {
    return {
      options: baseOptions,
...
async function runCloseLoopBatchGoals(goals, options) {
  ...
  const goalPlansByIndex = goalPlans.map(plan => ({
    ...
    run_options: { ...effectiveRunOptions }
  }));
...
function resolveControllerQueueFile(projectPath, queueFileCandidate) {
  const normalized = typeof queueFileCandidate === 'string' && queueFileCandidate.trim()
    ...
    : '.kiro/auto/controller-goals.lines';
...
async function loadControllerGoalQueue(projectPath, queueFileCandidate, formatCandidate) {
...
async function runCloseLoopController(queueFile, options = {}, context = {}) {
  const projectPath = context.projectPath || process.cwd();
  const queuePayload = await loadControllerGoalQueue(projectPath, queueFile, options.queueFormat);
  ...
  for (let cycle = 1; cycle <= maxCycles; cycle += 1) {
    ...
    const dequeuedGoals = pendingGoals.slice(0, dequeueLimit);
...
function normalizeAutoKpiTrendMode(modeCandidate) {
  const normalized = typeof modeCandidate === 'string'
    ...
    : 'all';
  if (!['all', 'batch', 'program', 'recover'].includes(normalized)) {
    throw new Error('--mode must be one of: all, batch, program, recover.');
  }
  return normalized;
}
...
async function buildAutoKpiTrend(projectPath, options = {}) {
  ...
  for (const session of sessions) {
    ...
    const successRate = Number(payload && payload.metrics && payload.metrics.success_rate_percent);
    const completionRate = Number(payload && payload.program_kpi && payload.program_kpi.completion_rate_percent);
    const failedGoals = Number(payload && payload.failed_goals);
    const totalSubSpecs = Number(payload && payload.metrics && payload.metrics.total_sub_specs);
    const estimatedSpecCreated = Number(payload && payload.spec_session_budget && payload.spec_session_budget.estimated_created);
