...
function buildMoquiRegressionRecoverySequenceLines(options = {}) {
  const clusterGoalsArg = options.clusterGoalsArg || AUTO_HANDOFF_MOQUI_CLUSTER_REMEDIATION_FILE;
  const baselineArg = options.baselineArg || AUTO_HANDOFF_MOQUI_BASELINE_JSON_FILE;
  ...
  return [
    ...
    `Step 1 (Cluster phased): ${formatCommand(`node scripts/moqui-matrix-remediation-phased-runner.js --cluster-goals ${clusterGoalsArg} --json`)}${suffix}`,
    `Step 1 alias: ${formatCommand('npm run run:matrix-remediation-clusters-phased -- --json')}${suffix}`,
    `Step 1 fallback (cluster batch): ${formatCommand(`sce auto close-loop-batch ${clusterGoalsArg} --format json --batch-parallel 1 --batch-agent-budget 2 --batch-retry-until-complete --json`)}${suffix}`,
    `Step 1 fallback alias: ${formatCommand('npm run run:matrix-remediation-clusters')}${suffix}`,
    `Step 2 (Baseline phased): ${formatCommand(`node scripts/moqui-matrix-remediation-phased-runner.js --baseline ${baselineArg} --json`)}${suffix}`,
    `Step 2 alias: ${formatCommand('npm run run:matrix-remediation-from-baseline -- --json')}${suffix}`
  ];
}
