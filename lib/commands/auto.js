...
function buildAutoHandoffRunRecommendations(projectPath, result) {
  ...
  if (result && result.remediation_queue && result.remediation_queue.file) {
    ...
    if (moquiCoverageRegressions.length > 0) {
      ...
      push('npm run run:matrix-remediation-clusters-phased -- --json');
      push(
      ...
      push('npm run run:matrix-remediation-clusters');
      push(
        `node scripts/moqui-matrix-remediation-phased-runner.js ` +
        `--baseline ${quoteCliArg(AUTO_HANDOFF_MOQUI_BASELINE_JSON_FILE)} --json`
      );
      push('npm run run:matrix-remediation-from-baseline -- --json');
    }
  }
