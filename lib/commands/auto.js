...
function buildGovernanceRecommendations(summary) {
  ...
  if (!handoffAvailable) {
    ...
    if (
      ...
      recommendations.push(
        'Step 1 alias: `npm run run:matrix-remediation-clusters-phased -- --json`.'
      );
      recommendations.push(
        'Step 1 fallback (cluster batch): ' +
      ...
      recommendations.push(
        'Step 2 alias: `npm run run:matrix-remediation-from-baseline -- --json`.'
      );
    }
  }
...
function buildGovernanceCloseLoopRecommendations(finalAssessment, stopReason, stopDetail) {
  ...
  if (reasons.some(item => `${item}`.startsWith('handoff-moqui-matrix-regressions-'))) {
    ...
    base.push('Step 1 alias: `npm run run:matrix-remediation-clusters-phased -- --json`.');
    base.push(
      'Step 1 fallback (cluster batch): ' +
      '`sce auto close-loop-batch .kiro/auto/matrix-remediation.capability-clusters.json ' +
      '--format json --batch-parallel 1 --batch-agent-budget 2 --batch-retry-until-complete --json`.'
    );
    base.push('Step 1 fallback alias: `npm run run:matrix-remediation-clusters`.');
    base.push(
      'Step 2 (Baseline phased): ' +
      '`node scripts/moqui-matrix-remediation-phased-runner.js ' +
      '--baseline .kiro/reports/release-evidence/moqui-template-baseline.json --json`.'
    );
    base.push('Step 2 alias: `npm run run:matrix-remediation-from-baseline -- --json`.');
  }
