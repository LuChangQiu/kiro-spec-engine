...
**核心理念**：
- 任何测试失败都可能是系统性问题的征兆
- "看起来无关"的测试失败往往隐藏着深层问题
- 忽视测试失败违背 Ultrawork 精神和 KSE 核心价值观
...
- ✅ **必须执行**: 调查每一个测试失败的根本原因
- ✅ **必须执行**: 修复所有测试失败后才能继续
- ✅ **必须执行**: 发布前运行完整测试套件并确保全部通过

**为什么重要**：
1. **质量保证**: 测试是代码质量的最后一道防线
2. **系统稳定性**: 小问题累积会导致系统崩溃
3. **用户信任**: 发布有问题的版本会损害用户信任
4. **技术债务**: 忽视问题会让未来修复成本指数级增长
