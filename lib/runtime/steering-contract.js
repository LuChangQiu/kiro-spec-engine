...
const LEGACY_STEERING_DIR = path.join('.kiro', 'steering');
...
class SteeringContract {
  constructor(workspaceRoot) {
    ...
    this._legacySteeringDir = path.join(workspaceRoot, LEGACY_STEERING_DIR);
  ...
  get legacySteeringDir() {
    return this._legacySteeringDir;
  }
  ...
  async ensureContract() {
    ...
    for (const [layerName, filename] of Object.entries(manifest.layers)) {
      ...
      const legacyPath = path.join(this._legacySteeringDir, filename);
      if (await fs.pathExists(legacyPath)) {
        await fs.copyFile(legacyPath, targetPath);
        preparedLayers.push({ layer: layerName, file: filename, source: 'kiro' });
        continue;
      }
  ...
  async resolveLayerFile(filename) {
    ...
    const legacyPath = path.join(this._legacySteeringDir, filename);
    if (await fs.pathExists(legacyPath)) {
      return { source: 'kiro', path: legacyPath };
    }
    return null;
  }
  ...
  _resolveSourceMode(sourceSet) {
    if (sourceSet.has('sce') && sourceSet.has('kiro')) {
      return 'mixed';
    }
    if (sourceSet.has('sce')) {
      return 'sce';
    }
    if (sourceSet.has('kiro')) {
      return 'kiro-compat';
    }
    return 'empty';
  }
