...
1. **Steering Exclusivity**: Kiro IDE loads all files in `.kiro/steering/`, requiring exclusive use of either kse or project steering rules
...
**Key Behaviors**:
- Creates `.kiro/workspace/{username}/` on init
...
*For any* adoption operation, the chosen steering strategy should be recorded in `.kiro/adoption-config.json`.

**Validates: Requirements 2.6**

### Property 5: Workspace Initialization

*For any* developer running `kse workspace init`, the system should create `.kiro/workspace/{username}/` with CURRENT_CONTEXT.md and task-state.json files.
...
*For any* spec, running `kse context export {spec-name}` should generate a Markdown file at `.kiro/specs/{spec-name}/context-export.md` containing requirements, design, tasks, and steering rules.

**Validates: Requirements 6.1, 6.2, 6.3, 6.4, 6.5**

### Property 14: Prompt Generation Completeness

*For any* task, running `kse prompt generate {spec-name} {task-id}` should create a prompt file at `.kiro/specs/{spec-name}/prompts/task-{task-id}.md` containing task description, relevant requirements, design sections, and status update instructions.
...
*For any* sync operation, the system should append an entry to `.kiro/workspace/{username}/sync.log` with timestamp and operation details.

**Validates: Requirements 9.5**

### Property 17: Single-User Mode Detection

*For any* project without `.kiro/workspace/` directory, the system should operate in single-user mode and preserve all existing functionality.
...
1. **Detection**: Check for `.kiro/workspace/` to determine mode
