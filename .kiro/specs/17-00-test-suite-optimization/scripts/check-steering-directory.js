 ...
 * 
 * Checks .kiro/steering/ directory for unauthorized files
...
async function checkSteeringDirectory() {
  const steeringPath = path.join(process.cwd(), '.kiro', 'steering');
  
  console.log('🔍 Checking .kiro/steering/ directory...\n');
  
  if (!await fs.pathExists(steeringPath)) {
    console.log('❌ Error: .kiro/steering/ directory not found');
    return false;
  }
  ...
  console.log('  1. Move analysis reports to .kiro/specs/{spec-name}/results/');
  console.log('  2. Move historical data to .kiro/specs/{spec-name}/');
  console.log('  3. Move detailed documentation to docs/');
  console.log('  4. Delete temporary files');
  console.log();
  console.log('⚠️  Remember: All files in .kiro/steering/ are auto-loaded in EVERY session!');
