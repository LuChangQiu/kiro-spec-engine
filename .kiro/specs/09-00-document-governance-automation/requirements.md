...
- **Spec_Directory**: A directory under `.kiro/specs/{spec-name}/`
...
1. WHEN the user runs `kse doctor --docs`, THE Diagnostic_Engine SHALL scan the Root_Directory for document violations
...
1. WHEN the user runs `kse cleanup`, THE Cleanup_Tool SHALL remove all temporary documents from the Root_Directory
2. WHEN the user runs `kse cleanup`, THE Cleanup_Tool SHALL remove all temporary documents from Spec_Directory structures
3. WHEN the user runs `kse cleanup --dry-run`, THE Cleanup_Tool SHALL display what would be deleted without actually deleting files
4. WHEN the user runs `kse cleanup --interactive`, THE Cleanup_Tool SHALL prompt for confirmation before deleting each file
5. WHEN the user runs `kse cleanup --spec {spec-name}`, THE Cleanup_Tool SHALL only clean the specified Spec directory
...
1. WHEN the user runs `kse validate`, THE Validation_Engine SHALL check Root_Directory document compliance
2. WHEN validating the Root_Directory, THE Validation_Engine SHALL verify only the four allowed .md files exist
3. WHEN the user runs `kse validate --spec {spec-name}`, THE Validation_Engine SHALL verify the specified Spec directory structure
...
8. WHEN the user runs `kse validate --all`, THE Validation_Engine SHALL validate all Spec directories in the project
...
1. WHEN the user runs `kse archive --spec {spec-name}`, THE Archive_Tool SHALL scan the Spec_Directory for unorganized artifacts
...
8. WHEN the user runs `kse archive --dry-run`, THE Archive_Tool SHALL display what would be moved without actually moving files
...
1. WHEN the user runs `kse hooks install`, THE Document_Governance_System SHALL install a pre-commit Git hook
...
6. WHEN the user runs `kse hooks uninstall`, THE Document_Governance_System SHALL remove the pre-commit hook
...
1. WHEN the user runs `kse config docs`, THE Document_Governance_System SHALL display current document governance settings
2. WHEN the user runs `kse config docs --set root-allowed-files "README.md,CUSTOM.md"`, THE Document_Governance_System SHALL update the list of allowed root directory files
3. WHEN the user runs `kse config docs --set spec-subdirs "reports,scripts,custom"`, THE Document_Governance_System SHALL update the list of recognized Spec subdirectories
...
6. WHEN the user runs `kse config docs --reset`, THE Document_Governance_System SHALL restore default document governance settings
...
2. WHEN the user runs `kse docs stats`, THE Document_Governance_System SHALL display document compliance statistics
...
6. WHEN the user runs `kse docs report`, THE Document_Governance_System SHALL generate a comprehensive compliance report in markdown format
7. WHEN generating a report, THE Document_Governance_System SHALL save it to `.kiro/reports/document-compliance-{timestamp}.md`
...
1. WHEN the user runs `kse --help`, THE Document_Governance_System SHALL display document governance commands in the help output
2. WHEN the user runs `kse doctor`, THE Document_Governance_System SHALL include document checks alongside other diagnostic checks
3. WHEN the user runs `kse status`, THE Document_Governance_System SHALL include document compliance status in the output
